...
- **1xx (Informational)**: Indica que a requisição foi recebida e está sendo processada. Exemplo: 100 Continue.
- **2xx (Successful)**: Indica que a requisição foi bem-sucedida. Exemplo: 200 OK, 201 Created.
- **3xx (Redirection)**: Indica que a requisição precisa de redirecionamento para outro recurso. Exemplo: 301 Moved Permanently, 302 Found.
- **4xx (Client Error)**: Indica que houve um erro por parte do cliente. Exemplo: 400 Bad Request, 401 Unauthorized, 404 Not Found.
- **5xx (Server Error)**: Indica que houve um erro por parte do servidor. Exemplo: 500 Internal Server Error, 502 Bad Gateway.
